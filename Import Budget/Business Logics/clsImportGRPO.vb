...
Public Class clsImportGRPO
    ...
    Public Sub LoadForm()
        Try
            oForm = oApplication.Utilities.LoadForm(xml_GRPOImp, frm_GRPOImp)
    ...
    Private Function ReadExcelData_New(ByVal aForm As SAPbouiCOM.Form, ByVal strID As String) As Boolean
        ...
        Try
            ...
            Try
                '  oTest.DoQuery("Delete from [Z_ODLN]")
            Catch ex As Exception

            End Try
            Dim strCode As String = oApplication.Utilities.getMaxCode("@S_GImport", "Code")
            Dim s1 As String = "Insert Into [@S_GImport] values('" & strCode & "','" & strCode & "')"
            oTest.DoQuery(s1)

            Dim oEdit As SAPbouiCOM.EditText
            oEdit = aForm.Items.Item("81").Specific
            oEdit.String = strCode
            oTest.DoQuery("Delete From [Z_ODLN] where RefCode='" & strCode & "'")
                    ...
                    For j As Integer = 1 To bound0
                        ...
                        Try
                            If j > 2 And array(j, 1).ToString <> "" Then
                                oApplication.Utilities.ReadExcelDat_GRPO(array(j, 1), array(j, 2), array(j, 3), array(j, 4), array(j, 5), array(j, 6), aForm, j - 1, strRefCode)
                            End If
                        Catch ex As Exception
            ...
            w.Close()
            excel.Quit()
            If Not IsNothing(_oDtGRPO) Then
                If _oDtGRPO.Rows.Count > 0 Then
                    ' Dim _unPivot As DataTable = unPivotTable(oForm, _oDt)
                    Dim strDtXML As String = oApplication.Utilities.getXMLstring(_oDtGRPO) ' _unPivot)
                    Dim s As String = "Exec [Insert_GRPOImport] '" + strDtXML + "'"
                    oRecordSet = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                    oRecordSet.DoQuery("Exec [Insert_GRPOImport] '" + strDtXML + "'")
                    Return True
                End If
            End If
        Catch ex As Exception
...
#Region "Item Event"
    Public Overrides Sub ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        Try
            If pVal.FormTypeEx = frm_GRPOImp Then
                        ...
                        Select Case pVal.EventType
                            Case SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED
                                    ...
                                    If CType(oForm.Items.Item("17").Specific, SAPbouiCOM.StaticText).Caption <> "" Then
                                        If oApplication.Utilities.ValidateFile(oForm, "56") Then
                                            If ReadExcelData_New(oForm, "56") Then ' oApplication.Utilities.GetExcelData(oForm, "56") Then
                                                ...
                                                oApplication.Utilities.Message("GRPO Data Imported Successfully....", SAPbouiCOM.BoStatusBarMessageType.smt_Success)
                                ...
                                ElseIf (pVal.ItemUID = "62") Then
                                    oForm = oApplication.SBO_Application.Forms.Item(FormUID)
                                    Dim _retVal As Integer = oApplication.SBO_Application.MessageBox("Proceed with creating a GRPO ?", 2, "Yes", "No", "")
                                    If _retVal = 1 Then

                                        Dim strQuery, strBatchNo As String
                                        Dim Otemp As SAPbobsCOM.Recordset
                                        Otemp = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                                        Dim oEdit As SAPbouiCOM.EditText
                                        oEdit = oForm.Items.Item("81").Specific
                                        strQuery = oEdit.String
                                        strBatchNo = "select ManBtchNum, * from [Z_ODLN] T0 Inner Join OITM T1 on T1.ItemCode=T0.Code where T1.ManBtchNum='Y' and T0.RefCode='" & strQuery & "'"
                                        Otemp.DoQuery(strBatchNo)
                                        If Otemp.RecordCount > 0 Then
                                            oEdit = oForm.Items.Item("80").Specific
                                            strBatchNo = oEdit.String
                                            If strBatchNo = "" Then
                                                oApplication.Utilities.Message("Batch Number is missing...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                                                Exit Sub
                                            End If
                                        End If

                                        UpdateRecords(oForm)
                                        If oApplication.Utilities.saveAsDraft_GRPO(oForm, strBatchNo) Then
                                            oApplication.Utilities.Message("GRPO  Document Created Sucessfully....", SAPbouiCOM.BoStatusBarMessageType.smt_Success)
                                            oForm.Close()
                                        Else
...
#Region "Function"
    ...
    Private Sub Initialize(ByVal oForm As SAPbouiCOM.Form)
        Try
            ...
            oForm.DataSources.UserDataSources.Add("BatchNum", SAPbouiCOM.BoDataType.dt_SHORT_TEXT)
            ...
            oEdit.String = "01"
            oEdit = oForm.Items.Item("80").Specific
            oEdit.DataBind.SetBound(True, "", "BatchNum")
    ...
    Private Sub loadCombo(ByVal oForm As SAPbouiCOM.Form)
        Try
            ...
            'Series Fill
            oComboBox = oForm.Items.Item("20").Specific
            strQuery = "Select Series,SeriesName From NNM1 Where ObjectCode = '20'"
    ...
    Private Function getUserSeries_GRPO(ByVal oForm As SAPbouiCOM.Form) As Integer
        ...
        Try
            ...
            oSeries = oSeriesService.GetDataInterface(SAPbobsCOM.SeriesServiceDataInterfaces.ssdiSeries)
            oDocumentTypeParams = oSeriesService.GetDataInterface(SAPbobsCOM.SeriesServiceDataInterfaces.ssdiDocumentTypeParams)
            oDocumentTypeParams.Document = 20
            oSeries = oSeriesService.GetDefaultSeries(oDocumentTypeParams)
            _retVal = oSeries.Series
            Return _retVal
        Catch ex As Exception
    ...
    Private Sub loadData(ByVal oForm As SAPbouiCOM.Form)
        Try
            ...
            oDt_Import = oForm.DataSources.DataTables.Item("Dt_Import")
            strQuery = " Select RowID,ORDNO,CODE,T1.[ItemName] As [DESCRIPTION], (Convert(Decimal(18,2),QTY)) as 'QTY',(Convert(Decimal(18,2),T0.Price)) 'Price',(Convert(Decimal(18,2),T0.Total)) 'Total' "
            strQuery += " ,BaseRef,BaseLine From Z_ODLN T0 "
            ...
            oDt_ErrorLog = oForm.DataSources.DataTables.Item("Dt_ErrorLog")
            strQuery = "Select RowID,CODE,T1.[ItemName] As [DESCRIPTION],ERROR As 'Error' From  Z_ODLN T0 left outer JOIN OITM T1 ON T0.CODE = T1.ItemCODE Where SUCCESS = '0' and RefCode='" & strCode & "'"
    ...
    Private Sub UpdateRecords(ByVal oForm As SAPbouiCOM.Form)
        oGrid = oForm.Items.Item("68").Specific
        Dim strQuery, strBatchNo, strREfNo As String
        Dim Otemp As SAPbobsCOM.Recordset
        Otemp = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        Dim oEdit As SAPbouiCOM.EditText
        oEdit = oForm.Items.Item("81").Specific
        strREfNo = oEdit.String
        strBatchNo = ""
        Otemp.DoQuery("select ManBtchNum, * from [Z_ODLN] T0 Inner Join OITM T1 on T1.ItemCode=T0.Code where T1.ManBtchNum='Y' and T0.RefCode='" & strREfNo & "'")
        If Otemp.RecordCount > 0 Then
            oEdit = oForm.Items.Item("80").Specific
            strBatchNo = oEdit.String
            If strBatchNo = "" Then
        ...
        End If
        For intRow As Integer = 0 To oGrid.DataTable.Rows.Count - 1
            strQuery = "Update Z_ODLN set Flag='Y',BatchNum='" & strBatchNo & "' where RefCode='" & strREfNo & "' and  RowID='" & oGrid.DataTable.GetValue("RowID", intRow) & "'"
